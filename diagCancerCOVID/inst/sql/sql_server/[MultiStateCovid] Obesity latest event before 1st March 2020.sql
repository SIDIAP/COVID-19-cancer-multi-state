...
FROM
(
  select E.person_id, E.start_date, E.end_date, row_number() OVER (PARTITION BY E.person_id ORDER BY E.start_date DESC) ordinal, OP.observation_period_start_date as op_start_date, OP.observation_period_end_date as op_end_date, cast(E.visit_occurrence_id as bigint) as visit_occurrence_id
  FROM 
  (
...
WHERE C.measurement_date < DATEFROMPARTS(2020, 03, 01)
AND (C.value_as_number >= 30.0000 and C.value_as_number <= 60.0000)
...
WHERE C.condition_start_date < DATEFROMPARTS(2020, 03, 01)
...
WHERE C.observation_date < DATEFROMPARTS(2020, 03, 01)
...
WHERE C.measurement_date < DATEFROMPARTS(2020, 03, 01)
AND (C.value_as_number >= 120.0000 and C.value_as_number <= 200.0000)
